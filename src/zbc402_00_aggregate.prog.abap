...
* calculate discount
  ...
  METHOD set_agencies.
    SELECT DISTINCT agencynum name city	FROM bc402_scus_book INTO TABLE mt_agencies WHERE customid = mv_customid ORDER BY name.

  ENDMETHOD.

  METHOD set_sums.

    SELECT SUM( forcuram ) forcurkey FROM sbook INTO TABLE mt_sums WHERE customid = mv_customid
      GROUP BY forcurkey.
